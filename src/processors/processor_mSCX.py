...
class Processor_mSCX(processor.Processor):
	...
	def process(self, channels={}):
		...
		for i in tqdm(range(num_epochs)):
			...
			for key in channels.keys():
				...
				for j in range(len(channels[key])):
					...
					if self.parameters.FEATURES['0.BANDS'][key]:
						bands = dataLib.computeBands(f, Pxx, self.parameters.FEATURES['0.BANDS'][key])
			...
			data_row = self.formatFeatures(features)
			data.append(data_row)
		np_data = np.array(data)

		np_data = np_data.transpose()
		for col in range(len(np_data)):
			np_data[col] = dataLib.maxNormalize(np_data[col], scale=(5,10))
			np_data[col] = dataLib.hullMovingAverage(np_data[col], 4)
			np_data[col] = dataLib.logTransform(np_data[col], np.median(np_data[col]))
		np_data = np_data.transpose()
		...
		for f in features:
			...
			for key in keys:
				if key != 'merge' and self.parameters.FEATURES[f][key] not in [False, None]:
					if isinstance(self.parameters.FEATURES[f][key], list) and f == '0.BANDS':
						headers.append(f+':'+key)
					elif isinstance(self.parameters.FEATURES[f][key], list):
